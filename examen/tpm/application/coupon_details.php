...
<!-- [ Header ] end -->
    ...
    <!-- [ Main Content ] start -->
    <div class="pc-container container py-5">
  <div class="pc-content row justify-content-center">
    <div class="col-md-8">
      <div class="card shadow-lg p-4 mb-4 border-0 rounded">
        <div class="card-header bg-primary text-white rounded-top">
          <h3 class="card-title mb-0 text-center">
            <i class="fas fa-tags"></i> <?= htmlspecialchars($coupon['name']) ?>
          </h3>
        </div>
        <div class="card-body">
          <ul class="list-unstyled mb-4">
            <li>
              <strong><i class="fas fa-barcode"></i> Código:</strong> <?= htmlspecialchars($coupon['code']) ?>
            </li>
            <li>
              <strong><i class="fas fa-percentage"></i> Descuento:</strong> <?= htmlspecialchars($coupon['percentage_discount']) ?>%
            </li>
            <li>
              <strong><i class="fas fa-money-bill-wave"></i> Monto Mínimo:</strong> <?= htmlspecialchars($coupon['min_amount_required']) ?>
            </li>
            <li>
              <strong><i class="fas fa-calendar-alt"></i> Fecha de Inicio:</strong> <?= htmlspecialchars($coupon['start_date']) ?>
            </li>
            <li>
              <strong><i class="fas fa-calendar-check"></i> Fecha de Fin:</strong> <?= htmlspecialchars($coupon['end_date']) ?>
            </li>
            <li>
              <strong><i class="fas fa-user-check"></i> Usos Máximos:</strong> <?= htmlspecialchars($coupon['max_uses']) ?>
            </li>
            <li>
              <strong><i class="fas fa-circle"></i> Estado:</strong> 
              <span class="<?= $coupon['status'] == 1 ? 'text-success' : 'text-danger' ?>">
                <?= $coupon['status'] == 1 ? 'Activo' : 'Inactivo' ?>
              </span>
            </li>
          </ul>
          <a href="coupon.php" class="btn btn-secondary btn-block">
